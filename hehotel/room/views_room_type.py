...
from com.mylib.response import conrender,HttpJsonResponse
...
from room.form import RoomTypeForm
...
def room_types(request,contype='html'):
    ...
    table = RoomTypeTable(
        ...
        order_by='0',
        reverse_accessors = {
            'action':lambda x:RoomTypeTable.get_action(x,request.user)
            }
    )
    print table.get_columns()
    print table.get_rows()
    return conrender(contype,{'html':('room/room_types.html',locals()),
                      'table':table,
                      'csv':(u"房间类别列表.csv",table),
                      })
